...
jointModel_Subdistr_Misc = function(fitlme,fitCoxCause1,fitCoxCause2,prior,startingValues,nknotsCause1 = 3,nknotsCause2 = 3,
                                    scaleVarSurv = 1,iterMaxSurv_1 = 1,iterMaxSurv_2 = 1,DoubleSamplVar = "Robs",TrueStatusVar = "status",
                                    full.knotsCause1 = NULL,full.knotsCause2 = NULL,timeVar = "times",ndraw = 100,thin = 1,store.b = F,nburn = 0,
                                    useGauleg = T,Gauleg_points = 30,tdf = 10)
{
  #########################################################################
  ...
  fDer = function(paramSurv,iter = 5)
  {
    paramCause1 = paramSurv[1:lpCause1]
    paramCause2 = paramSurv[-c(1:lpCause1)]
    ...
    score1 = .colSums(deltaCause1*xCause1Comb,G,lpCause1,F) - .colSums( deltaCause1Long*qq1Mat,nLong,lpCause1,F) 
    score1 = score1 - .colSums( (1 - deltaLong)*qq1Mat*aa1,nLong,lpCause1,F)
    
    score2 = .colSums(deltaCause2*xCause2Comb,G,lpCause2,F) - .colSums( deltaCause2Long*qq2Mat,nLong,lpCause2,F ) 
  ...
  fDer1 = function(paramCause1,iter = 5)
  {
    # paramCause1 = paramSurv[1:lpCause1]
    # paramCause2 = paramSurv[-c(1:lpCause1)]
  ...
  fDer2 = function(paramCause2,iter = 5)
  {
    # paramCause1 = paramSurv[1:lpCause1]
    # paramCause2 = paramSurv[-c(1:lpCause1)]
  ...
  fsurvEvalRdouble = function(paramSurv)
  {
    paramCause1 = paramSurv[1:lpCause1]
    paramCause2 = paramSurv[-c(1:lpCause1)]
  ...
  fsurvDer = function(paramSurv)
  {
    ...
    score1 = .colSums( deltaCause1*(xCause1Comb-s1der) - (1 - delta)*(s1der*tt1) ,G,lpCause1,F) 
    score2 = .colSums( deltaCause2*(xCause2Comb-s2der) - (1 - delta)*(s2der*tt2) ,G,lpCause2,F) 
    
    hess1 = crossprod(xCause1CombLong*(sqrt(qq1)*deltaCause1Long))
    hess1 = hess1 + crossprod( xCause1CombLong*(sqrt(qq1*aa1)*(1 - deltaLong)) ) - crossprod( s1der*sqrt(tt1)*(1-delta) ) + crossprod(s1der*tt1*(1-delta))
    
    hess2 = crossprod(xCause2CombLong*(sqrt(qq2)*deltaCause2Long))
    hess2 = hess2 + crossprod( xCause2CombLong*(sqrt(qq2*aa2)*(1 - deltaLong)) ) - crossprod( s2der*sqrt(tt2)*(1-delta) ) + crossprod(s2der*tt2*(1-delta))
    
    # lpcause1*lpcause2
    hess12 = crossprod(s1der*tt1*(1-delta),s2der*tt2)
  ...
  fitStart = survModel_Subdistr_addConstr(fitCox1_temp,fitCox2_temp,fitlme,Lbeta = Lbeta,reffects = b,timeVar = timeVar,
                                          nknotsCause1 = nknotsCause1,nknotsCause2 = nknotsCause2,useGauleg = useGauleg,Gauleg_points = Gauleg_points)
  ...
  for (j in 1:5)
  {
    ...
    fitStart_approx = survModel_Subdistr_addConstr(fitCox1_temp,fitCox2_temp,fitlme,Lbeta = Lbeta_lmm,reffects = b_lmm,timeVar = timeVar,
                                                   nknotsCause1 = NULL,nknotsCause2 = NULL,full.knotsCause1 = c(bknotsCause1[1],knotsCause1,bknotsCause1[2]),
                                                   full.knotsCause2 = c(bknotsCause2[1],knotsCause2,bknotsCause2[2]),
    ...
    for (i in 1:G)
    {
      ...
      for (j in 1:1)
      {
        ...
        if (deltaCause1[i]==1)
        {
          ...
          score = score + assoc1*zTime[i,] - s1derb
          
          # Information matrix
          inf = Linf + (0.5*times[i]*assoc1^2)*crossprod(ziLong*sqrt(ints1*wws))
        ...
        if (deltaCause2[i]==1)
        {
          ...
          # The score vector of f(bi|others)
          score = score + assoc2*zTime[i,] - s2derb
          
          # Information matrix
          inf = Linf + (0.5*times[i]*assoc2^2)*crossprod(ziLong*sqrt(ints2*wws))
        }
        ...
        if (delta[i]==0)
        {
          ...
          # The score vector of f(bi|others)
          scoreSurv = (es1b*s1derb + es2b*s2derb)/(es1b+es2b-1)
          score = score - scoreSurv
          
          inf = Linf + (- tcrossprod(s1derb)*es1b + es1b*(0.5*times[i]*assoc1^2)*crossprod(ziLong*sqrt(ints1*wws)) )/(es1b+es2b-1)
          inf = inf +  (- tcrossprod(s2derb)*es2b + es2b*(0.5*times[i]*assoc2^2)*crossprod(ziLong*sqrt(ints2*wws)) )/(es1b+es2b-1)
      ...
      if (deltaCause1[i]==1)
      {
        ...
        logp = logp + assoc1*sum(bdiff*zTime[i,]) - (s1bcan-s1b) - sum(bdiff*Linfmu1)
        
        st = es1bcan + es2bcan - 1
        if ( st < 0)
        {
          logp = - Inf
        }
      ...
      if (deltaCause2[i]==1)
      {
        ...
        logp = logp + assoc2*sum(bdiff*zTime[i,]) - (s2bcan-s2b) - sum(bdiff*Linfmu1)
      ...
      if (delta[i]==0)
      {
        ...
        logp = logp + log(es1bcan+es2bcan-1) - log(es1b+es2b-1) - sum(bdiff*Linfmu1)
      }
...
DICSubdistr_Misc = function(fitProp,nMC = 100,thinDIC = 10)
{
  # 7-15 Gauss-Kronrod quadradure points and weights
  gaussKronrod <-
    function (k = 15) {
...
predictLatentStates_Subdistr = function(fitProp,newdata1,newdata2,tt = seq(0,10,by = 2),nMC = 1000,seed = 12,thin = 1,nburn = 200)
{
  # if (class(fitProp)!=T)
  # {
  #   
  ...
  #   
  #   temp = cbind(1,lspline(timesForInt,knots = c(1,5)))
  #   tempid = cbind(1,lspline(time,knots = c(1,5)))
  #   
  #   # The design matrices at the additional points
  #   xTimeLong = temp
  #   xTimeid = tempid
  ...
  #   colnames(splBasisCause1) = NULL
  # 
  #   splBasisCause1id = bSpline(time,knots = knotsCause1,Boundary.knots = bknotsCause1,intercept = T)
  #   colnames(splBasisCause1id) = NULL
  # 
  #   # The true marker value at all points needed for integration
  #   mtpredLong = c(xTimeLong %*% Lbeta) + zTimeLong %*% br
  #   mtpredid = c(xTimeid %*% Lbeta) + sum(zTimeid*br)
  # 
  #   xbetasCause1 = c(w1 %*% betasCause1[-lbetasCause1]) + mtpredLong*betasCause1[lbetasCause1]
  #   uCause1 = c(splBasisCause1 %*% gammasCause1)
  #   linpredCause1 = xbetasCause1 + uCause1
  ...
  #   qq1 = (wws*t1Long)*expLinpredCause1
  # 
  #   xbetasCause1id = c(w1 %*% betasCause1[-lbetasCause1]) + mtpredid*betasCause1[lbetasCause1]
  #   uCause1id = c(splBasisCause1id %*% gammasCause1)
  #   linpredCause1id = xbetasCause1id + uCause1id
  #   expLinpredCause1id = exp(linpredCause1id)
  # 
  #   exp(-sum(qq1))*expLinpredCause1id
  # }
  ...
  # 
  #   # The design matrices at the additional points
  #   xTimeLong = model.matrix(formula(fitlme),data = dd)
  #   colnames(xTimeLong) = NULL
  #   rownames(xTimeLong) = NULL
  # 
  #   xTimeid = model.matrix(formula(fitlme),data = ddid)[1,]
  #   colnames(xTimeid) = NULL
  #   rownames(xTimeid) = NULL
  # 
  #   zTimeLong = model.matrix(formula(fitlme$modelStruct$reStr)[[1]],dd)
  #   colnames(zTimeLong) = NULL
  #   rownames(zTimeLong) = NULL
  # 
  #   zTimeid = model.matrix(formula(fitlme$modelStruct$reStr)[[1]],ddid)[1,]
  #   colnames(zTimeid) = NULL
  #   rownames(zTimeid) = NULL
  # 
  #   # Construct the spline basis for cause 2
  #   splBasisCause2 = bSpline(timesForInt,knots = knotsCause2,Boundary.knots = bknotsCause2,intercept = T)
  #   colnames(splBasisCause2) = NULL
  # 
  #   splBasisCause2id = bSpline(time,knots = knotsCause2,Boundary.knots = bknotsCause2,intercept = T)
  #   colnames(splBasisCause2id) = NULL
  # 
  #   # The true marker value at all points needed for integration
  #   mtpredLong = c(xTimeLong %*% Lbeta) + zTimeLong %*% br
  #   mtpredid = c(xTimeid %*% Lbeta) + sum(zTimeid*br)
  ...
  #   qq2 = (wws*t1Long)*expLinpredCause2
  # 
  #   xbetasCause2id = c(w2 %*% betasCause2[-lbetasCause2]) + mtpredid*betasCause2[lbetasCause2]
  #   uCause2id = c(splBasisCause2id %*% gammasCause2)
  #   linpredCause2id = xbetasCause2id + uCause2id
  #   expLinpredCause2id = exp(linpredCause2id)
  # 
  #   exp(-sum(qq2))*expLinpredCause2id
  # }
